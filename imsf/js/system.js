...
function GetLocSID(){
	var re_resl = window.location.href.match(/\/view\/(.+?).html/);
	if(!re_resl){
		re_resl = window.location.href.match(/\/play\/(.+?)-\d+-\d+.html/);
	}
	if(!re_resl){
		return null;
	}
...
function IsMobile() {
	if (navigator.userAgent.toLowerCase().match(/(iphone|ipod|android|midp|windows mobile|windows ce|rv:1.2.3.4|ucweb)/i)) {
		return true;
	}
	return false;
}
...
var feifei = {
		...
		'flickity':function(){//手机滑动
			if($(".ff-gallery").length){
				...
				$("<link>").attr({
					rel: "stylesheet",
					type: "text/css",
					href: "https://cdn.jsdelivr.net/gh/imysoss/imoss6/imsf/Public/jquery.flickity/2.1.1/flickity.min.css"
				}).appendTo("head");
				$.getScript("https://cdn.jsdelivr.net/gh/imysoss/imoss6/imsf/Public/jquery.flickity/2.1.1/flickity.pkgd.min.js", function(){
	...
	'image': {//图片
		'lazyload': function(){//延迟加载
			...
			$.getScript("https://cdn.jsdelivr.net/gh/imysoss/imoss6/imsf/Public/jquery.lazyload/1.9.7/jquery.lazyload.min.js", function(response, status) {
				$("img.ff-img").lazyload({
					placeholder : "https://cdn.jsdelivr.net/gh/imysoss/imoss6/imsf/Public/images/no.jpg",
					effect : "fadeIn",
					failurelimit: 15
					//threshold : 400
	...
	'search': {//搜索
		...
		'autocomplete': function(){//3.5增加搜索结果控制 4.1update
			...
			if( $limit > 0){
				...
				$.getScript("https://cdn.jsdelivr.net/gh/imysoss/imoss6/imsf/Public/jquery.devbridge-autocomplete/1.4.7/jquery.autocomplete.min.js", function(response, status) {
					$('.ff-wd').autocomplete({
					...
					$(".ff-score").each(function(i){
						$(".ff-score").eq(i).find('.ff-score-raty').raty({ 
							...
							click: function(score, evt) {
								...
								$.ajax({
									type: 'get',
									url: cms.root+'pingfen?pf_num='+(score*2+0.2)+'&from='+GetLocSID(),
									timeout: 5000,
									dataType:'json',
									error: function(){
									...
									success: function(json){
										$this.parent().find('.ff-score-val').html(json.pf_num.toFixed(1));
										$this.parent().find('.re-pf-cnt-num').html(json.pf_cnt);
										if(json.msg){
											alert(json.msg);
										}										
									}
		...
		'more':function(){//...效果
			...
			$('.ff-playurl').each(function(i){
				...
				if(($config+2) < $max && $config>0){
					$max_html = $($this.find('li:last').prop("outerHTML")).find('a').attr('href','#all').html('展开>>').css('color', '#E00');
	...
	$(document).ready(function(){
		var curr_href = self.location.href;
		if(IsMobile()){
			if(curr_href.match(/^https*:\/\/www\./)){
				self.location.href = curr_href.replace("//www.", "//m.");
			}
		}else {
			//跳回PC端
			if(curr_href.match(/^https*:\/\/m\./)){
				self.location.href = curr_href.replace("//m.", "//www.");
			}
		}
		//
		feifei.cms.nav();//
